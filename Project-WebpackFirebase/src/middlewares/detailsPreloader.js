import { getByQuery, getCollectionReference, getSingleDoc } from '../server';
...
export const detailsPreloader = async (ctx, next) => {
    ...
    ctx.photo.comments = await getByQuery(
        collectionReference,
        'photoId',
        '==',
        ctx.params.id
    ...
    ctx.photo.hasLiked = ctx.user ? likes.some((like) => like.userId == ctx.user.uid) : false;
    
    ctx.photo.showLikeButton = !ctx.photo.hasLiked && !ctx.photo.isOwner && ctx.hasUser
    next();
};
